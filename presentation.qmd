...
This research aims to investigate the relationship between specific types of 311 complaints and the boroughs they are most common in, as well as the demographic characteristics of those boroughs. This research is important as it can help identify potential areas for improvement in service delivery and policy development in New York City. Previous studies have analyzed the relationship between 311 complaints and various demographic factors such as income, race, and education level. However, there is a need to further explore the variations in 311 complaints across different boroughs.
...
The data for this study was collected from the NYC Open Data Portal, which contains information on all 311 service requests received by the City of New York since 2010. The data set will be filtered to include only the complaints from January 2023. The sampling scheme include all 311 complaints received by all Agency's. The data set will include information on the borough, complaint type, and other relevant demographic information such as population density, and median household income.
...
::: {.panel-tabset}
### Code
What if we want to visualize the Top 20 Complaints Per Bourough?:
```{python}
#| echo: true
#| eval: false
...
::: {.panel-tabset}
### Code
Lets create a contingency table to perform more statistical tests:
```{python}
#| echo: true
#| eval: false
...
Now lets test it using a Chi-Sqaured test for Contintency:
...
## Visualizations
Lets check if we can see this visually with a heatmap:
...
The ANOVA test only tells us that there are differences between the groups, but not which groups are different from each other. To explore this further, we can perform a post-hoc test known as Tukey's HSD test to identify which groups are significantly different from each other.
...
## Modeling
Finally, we can change the dependent variable, `Borough` to numerical values:
...
In terms of data in reference to Borough and Complaint Type:

All combinations of boroughs have signifincantly different results except for bronx vs brooklyn, and manhattan vs staten island. Therefore, we can conlclude that the distribution of complaints types across all combinations of boroughs are different (except the two noted above). Additionally, every complaint is signifincantly different among all boroughs. 

In terms of data in reference to Borough and Demograpics:

Each demographic variable is different between each set of boroughs inlcuding the boroughs that had similar complaint types as we conluded previously. 

In terms of Complaint Type and Demographic:

There are many combinations in which based on the specific demographic, the difference between complaint types are not significant, but for the most part, they are statistically significant.

In terms of the neural network model:

Because of our previous statistical conclusions, using demographic variables along with the other variables such as Complaint Type and duration, we would expect a model to be able to accurately predict which borough the complaint was coming from. And, although the borough of the request is known upon recieving the call, the implications of the predictors themselves can guide future research.

Based on the results, the model was able to predict with strong accuracy which borough the complaints were coming from.

Overall, I would suggest that it is worthwile for local policy makers across boroughs to identify which complaints are most common in their area and allocate resources accordingly in order to save time and money. In addition to this, based on the significance of demographic variables and their relationship with specific complaint types, I think further reasearch could be done to associate these complaints to specific demographics and utilize that information accordingly.